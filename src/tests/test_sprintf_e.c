...
START_TEST(high_degree) {
  char str1[100];
  char str2[100];
  double d = 0.00000000003L;
  char format[] = "!%.3e!";
  sprintf(str1, format, d);
  s21_sprintf(str2, format, d);
  ck_assert_str_eq(str1, str2);
...
Suite* s21_print_e_suite(void) {
  ...
  tcase_add_test(tc_core, small_number);
  tcase_add_test(tc_core, zero_number);
  tcase_add_test(tc_core, high_degree);
